...
public class AndroidOperate extends AppOperate{
	...
	public AndroidOperate(AndroidDriver<WebElement> driver) {
		super(driver);
	...
	public boolean swipeToLeft() {
		Log.logInfo("模拟向左滑动");
		int width=((AppiumDriver<WebElement>)driver).manage().window().getSize().width;
		int height=((AppiumDriver<WebElement>)driver).manage().window().getSize().height;
		Log.logInfo(width);
		Log.logInfo(height);
		try{
		((AppiumDriver<WebElement>)driver).swipe(width * 8 / 10, height* 9/ 10, width * 2/ 10, height *9/ 10, 1000);
		}catch(Exception e)
		{
			return false;
		}
	...
	public boolean swipeToRight() {
		...
		Log.logInfo(height);
		try{
		((AppiumDriver<WebElement>)driver).swipe(width * 2 / 10, height* 9/ 10, width * 8/ 10, height *9/ 10, 1000);
		}catch(Exception e)
		{
			return false;
		}
...
//					}
	...
	{
		((AppiumDriver<WebElement>) driver).findElementByName("完成").click();
	}
		...
		while(true)
		{
			if (waitForText(3, "loading")) {
				...
				if (waitForText(3, "网络无法连接")) {
					click(driver.findElement(By.name("网络无法连接")), "尝试重连网络");
					Sleep.sleep(10);
				}else
				{
					break;
